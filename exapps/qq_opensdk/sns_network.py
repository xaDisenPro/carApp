...
from qq_opensdk.sns_sig import hmac_sha1_sig
...
class SNSNetwork(object):
    ...
    def _http_send(self, method, url_path, ec_params):
        ...
        if method.lower() == 'post':
            data = requests.post(uri, ec_params).read()
        elif method.lower() == 'get':
            if ec_params:
                dest_url = '%s?%s' % (uri, ec_params)
            else:
                dest_url = uri
            data = requests.get(dest_url).read()
        else:
    ...
    def _https_send(self, method, url_path, ec_params):
        conn =urllib3.HTTPConnectionPool(choice(self._iplist)) # HTTPSConnection()
